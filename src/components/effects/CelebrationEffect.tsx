...
function EncouragementParticles({ active }: { active: boolean }) {
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < COUNT; i++) {
      // Movimiento sinusoidal suave hacia arriba
...
// ─── Confetti épico (score >= 50) ─────────────────────────────────────────
function EpicConfetti({ score, active }: Props) {
  const mesh = useRef<THREE.Points>(null!);
  // Más partículas y más rápidas cuanto mayor el puntaje
  ...
  const [positions, vel, colorArr] = useMemo(() => {
    ...
    for (let i = 0; i < COUNT; i++) {
      // Para score >= 90: explotan desde múltiples puntos (fuegos artificiales)
      // Para el resto: llueven desde arriba
      if (score >= 90) {
  ...
  useFrame(() => {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      pos[i * 3 + 1] += vel[i * 3 + 1];
      pos[i * 3 + 2] += vel[i * 3 + 2];
      vel[i * 3 + 1] -= score >= 90 ? 0.0015 : 0.0005; // gravedad
      // Reiniciar partículas que salen de escena
